...
export default function DashboardPage() {
  ...
  // 🔄 Track toggle states for each break
  ...
  // 🔁 Map to correct LogType enum
  const logKeyMap = {
    lunch: 'LUNCH_BREAK',
    prayer: 'PRAYER_BREAK',
    leaveDesk: 'LEAVE_DESK',
  } as const;
  ...
  // 🔘 Break toggling
  const toggleBreak = async (type: 'lunch' | 'prayer' | 'leaveDesk') => {
    const isActive = breakStatus[type];
    const logType = `${logKeyMap[type]}_${isActive ? 'END' : 'START'}` as
      | 'LUNCH_BREAK_START' | 'LUNCH_BREAK_END'
      | 'PRAYER_BREAK_START' | 'PRAYER_BREAK_END'
      | 'LEAVE_DESK_START' | 'LEAVE_DESK_END';

    const res = await fetch('/api/log-break', {
      method: 'POST',
    ...
    if (res.ok) {
      setBreakStatus({ ...breakStatus, [type]: !isActive });
      alert(`${logType.replace(/_/g, ' ')} logged successfully`);
    } else {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-100 space-y-6">
      ...
      <h1 className="text-2xl font-bold mt-20">Welcome, {employee.email}</h1>
      ...
      >
        {breakStatus.lunch ? 'Back from Lunch' : 'Start Lunch Break'}
      </button>

      {/* 🙏 Prayer Break */}
      <button
        className="bg-blue-600 text-white px-6 py-2 rounded hover:bg-blue-700"
        onClick={() => toggleBreak('prayer')}
      >
        {breakStatus.prayer ? 'Back from Prayer' : 'Start Prayer Break'}
      </button>

      {/* 💺 Leave Desk */}
      <button
        className="bg-gray-700 text-white px-6 py-2 rounded hover:bg-gray-800"
        onClick={() => toggleBreak('leaveDesk')}
      >
        {breakStatus.leaveDesk ? 'Back to Desk' : 'Leave Desk'}
      </button>
    </div>
  );
}
